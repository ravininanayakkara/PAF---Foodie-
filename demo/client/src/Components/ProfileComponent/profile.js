...
function Profile() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      await axios
        .get("http://localhost:8080/user/" + localStorage.getItem("name"))
        .then((user) => {
          console.log(user);
          setIsLoading(false);
          setdetails(user);
  ...
  //pro pic/bio upload be eke api call wenwa
  ...
  function handleBio() {
    axios
      .put(
        "http://localhost:8080/user/bio/" + localStorage.getItem("userId"),
        bioS
      )
      .then((upload) => {
        console.log(upload);
