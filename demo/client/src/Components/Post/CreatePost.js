...
function CreatePost() {
  ...
  const handleSubmit = (event) => {
    ...
    try {
      //Todatabase
      axios
        .post("http://localhost:8080/post/upload", user)
        .then((response) => {
          console.log(response.data);
          handlePostId(response.data);
          const newReact = { ...react, postId: response.data };
          axios
            .post("http://localhost:8080/react/set", newReact)
            .then((response) => {
              console.log(response.data);
            })
            .then(() => {
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Space direction="horizontal" style={{ marginTop: "70px" }}>
