...
function ReactComponent(props) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await axios.get(
        `http://localhost:8080/react/${props.data.id}`
      );
      console.log(response.data);
      const { hahaCount, heartCount, sadCount, wowCount, id } =
        response.data[0];
  ...
  function handleHaha() {
    if (hahaFirst) {
      ...
      //updateHaha
      axios
        .put(`http://localhost:8080/react/${reactId}`, {
          hahaCount: hahaCount1 + 1,
        })
        .then((response) => {
          console.log(response);
          handleNotification();
  ...
  function handleHeart() {
    if (heartFirst) {
      ...
      setHeartFirst(false);
      axios
        .put(`http://localhost:8080/react/heart/${reactId}`, {
          heartCount: heartCount1 + 1,
        })
        .then((response) => {
          console.log(response);
          // Handle successful response here
